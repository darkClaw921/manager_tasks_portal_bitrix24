...
/**
 * Owner-only modal for inviting users to a workspace.
 *
 * Pattern lifted from `src/components/meetings/InviteParticipantsModal.tsx`
 * with `meetingId` → `workspaceId` and an extra role select (editor / viewer).
 * Uses `/api/meetings/invitable-users` since that endpoint already returns
 * the right minimal user list and is the same audience.
 */
...
import { useInvitableUsers } from '@/hooks/useMeeting';
import { useAddWorkspaceParticipants } from '@/hooks/useWorkspace';
...
export function InviteParticipantsModal({
  ...
  const invite = useAddWorkspaceParticipants(workspaceId);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await invite.mutateAsync({ userIds: Array.from(selected), role });
