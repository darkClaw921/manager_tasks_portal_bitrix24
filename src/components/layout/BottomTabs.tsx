...
function PortalsIcon({ active }: { active: boolean }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill={active ? 'currentColor' : 'none'} viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
...
const TABS: TabItem[] = [
  ...
  },
  {
    href: '/portals',
    label: 'Порталы',
    icon: (active) => <PortalsIcon active={active} />,
  },
  {
    href: '/reports',
    label: 'AI',
      ...
      {TABS.map((tab) => {
        ...
        return (
          <Link
            ...
            href={tab.href}
            className={cn(
              'flex flex-col items-center gap-0.5 px-3 py-2 min-w-[56px] min-h-[44px] transition-colors',
