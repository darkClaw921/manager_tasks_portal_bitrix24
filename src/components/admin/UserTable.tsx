...
export function UserTable({ users, currentUserId, onEdit, onDelete, onViewDetails, onChangePassword, isDeleting }: UserTableProps) {
                ...
                {editingId === user.id ? (
                  ...
                  <>
                        ...
                        <div>
                          <p className="text-small font-medium text-foreground">
                            ...
                            {user.id === currentUserId && (
                              <span className="text-xs text-text-muted ml-1">(вы)</span>
                            )}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {onViewDetails && (
                          <Button size="sm" variant="ghost" onClick={() => onViewDetails(user)} title="Подробнее">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => startEdit(user)} title="Редактировать">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                            <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                          </svg>
                        </Button>
                        {onChangePassword && (
                          <Button size="sm" variant="ghost" onClick={() => onChangePassword(user)} title="Сменить пароль">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                        ...
                        {user.id !== currentUserId && (
                          deleteConfirmId === user.id ? (
                            ...
                            <Button size="sm" variant="ghost" onClick={() => setDeleteConfirmId(user.id)} title="Удалить">
        ...
        {users.map((user) => (
          <div key={user.id} className="bg-surface rounded-card border border-border p-4">
            ...
            <div className="flex items-center justify-between text-xs text-text-secondary mb-3">
              <span>{user.portalCount} {user.portalCount === 1 ? 'портал' : user.portalCount >= 2 && user.portalCount <= 4 ? 'портала' : 'порталов'}</span>
            ...
            <div className="flex items-center gap-2 border-t border-border pt-3">
              ...
              {onChangePassword && (
                <Button size="sm" variant="ghost" onClick={() => onChangePassword(user)} className="flex-1">
                  Пароль
                </Button>
              )}
              {user.id !== currentUserId && (
