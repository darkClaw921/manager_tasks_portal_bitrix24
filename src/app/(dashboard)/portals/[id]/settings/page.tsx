...
function GeneralTab({ portal }: { portal: { id: number; name: string; domain: string; color: string; isActive: boolean; lastSyncAt: string | null; createdAt: string } }) {
  return (
    <div className="bg-surface rounded-card border border-border p-6">
      <h3 className="text-h3 font-semibold mb-4">Информация о портале</h3>

      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-small font-medium text-text-secondary mb-1">Название</label>
            <p className="text-body text-foreground">{portal.name}</p>
          </div>
          <div>
            <label className="block text-small font-medium text-text-secondary mb-1">Домен</label>
            <p className="text-body text-foreground">{portal.domain}</p>
          </div>
          <div>
            <label className="block text-small font-medium text-text-secondary mb-1">Цвет</label>
          ...
          <div>
            <label className="block text-small font-medium text-text-secondary mb-1">Статус</label>
            <Badge variant={portal.isActive ? 'success' : 'danger'} size="md">
              {portal.isActive ? 'Активен' : 'Неактивен'}
            </Badge>
          </div>
          <div>
            <label className="block text-small font-medium text-text-secondary mb-1">Последняя синхронизация</label>
            <p className="text-body text-foreground">
              {portal.lastSyncAt ? new Date(portal.lastSyncAt).toLocaleString() : 'Никогда'}
            </p>
          </div>
          <div>
            <label className="block text-small font-medium text-text-secondary mb-1">Создан</label>
            <p className="text-body text-foreground">
...
function MappingsTabContent({ portalId }: { portalId: number }) {
  ...
  const handleCreateMapping = useCallback(async (data: { userId: number; bitrixUserId: string; bitrixName?: string }) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(error.message || 'Не удалось создать привязку');
    }
    // Force refetch
  ...
  const handleDeleteMapping = useCallback(async (userId: number) => {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || 'Не удалось удалить привязку');
    }
  }, [portalId]);
...
function SettingsContent() {
    ...
    return (
      <div className="max-w-5xl mx-auto">
        <div className="text-center py-12">
          ...
          <h2 className="text-h3 font-semibold text-foreground mb-2">Доступ запрещён</h2>
          <p className="text-text-secondary text-small mb-4">
            Для доступа к этой странице необходимы права администратора портала.
          </p>
  ...
  if (!portal) {
    return (
      <div className="max-w-5xl mx-auto">
        <div className="text-center py-12">
          <h2 className="text-h3 font-semibold text-foreground mb-2">Портал не найден</h2>
          <p className="text-text-secondary text-small mb-4">
            Портал не существует или у вас нет доступа.
          </p>
          <Button variant="secondary" onClick={() => router.push('/portals')}>
            К порталам
          </Button>
